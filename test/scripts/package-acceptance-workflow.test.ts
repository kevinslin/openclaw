...
describe("package artifact reuse", () => {
  ...
  it("allows only the fixed fork historical extended-stable canary without changing upstream defaults", () => {
    const npmWorkflow = readFileSync(".github/workflows/openclaw-npm-release.yml", "utf8");

    expect(npmWorkflow).toContain("fork_core_npm_only:");
    expect(npmWorkflow).toContain("historical_extended_stable_test:");
    ...
    expect(npmWorkflow).toContain("dev/kevinlin/integ-extended-stable-2000-4");
    expect(npmWorkflow).toContain("@kevins8/openclaw");
    expect(npmWorkflow).toContain(
